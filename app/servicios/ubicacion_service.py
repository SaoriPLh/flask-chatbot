...
# Cargar las variables de entorno desde .env (solo localmente)
# load_dotenv()
...
def geocodificar_direccion(direccion):
    try:
        ...
        # Log de la respuesta
        print(f"Respuesta completa de la API de Google: {data}")

        if response.status_code == 200 and data.get('results'):
            location = data['results'][0]['geometry']['location']
            print(f"Coordenadas obtenidas: {location['lat']}, {location['lng']}")
            return location['lat'], location['lng']
        else:
            print(f"Error en la geocodificación: {data.get('error_message', 'Sin mensaje de error')}")
            return None
    ...
    try:
        ...
        for nombre_sucursal, area in sucursales.items():
            if area.contains(punto_cliente):
                print(f"Sucursal asignada: {nombre_sucursal}")
                return nombre_sucursal
        print("No se encontró una sucursal para las coordenadas dadas.")
        return None
    except Exception as e:
        print(f"Error al asignar sucursal: {e}")
        return None